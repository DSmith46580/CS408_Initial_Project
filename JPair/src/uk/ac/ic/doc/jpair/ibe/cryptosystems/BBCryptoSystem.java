...
public class BBCryptoSystem {
	...
	/**
	 * The Setup algorithm. This algorithm generates pseudo random
	 * PublicParameters and the Master Secret.
	 * 
	 * @param n
	 *            - Security Parameter which must be equal to 1024, 2048, 3072,
	 *            7680 or 15360
	 * @return PP - A set of public parameters (sstate, p, q, P, P_prime, P_1,
	 *         P_2, P_3, v, hashfcn) and the corresponding Master Secret 'S'. S
	 ...
	 * algorithms HashToRange in order to return an integer.
	 * 
	 * @param ID
	 *            - Identity String id
	 * @param pp
	 *            - A set of PublicParameters.
	 * @return H_ID - A BigInteger H_ID modulo q
	 * @throws NoSuchAlgorithmException
	 * 
	 * 
	 */
	 ...
	 * @param pp
	 *            - A set of PublicParameters.
	 * @return PrivateKey - A pair of points (D_0, D_1), each of which has order
	 *         q in E(F_p)
	 * @throws NoSuchAlgorithmException
	 * 
	 * 
	...
	/**
	 * The Encryption algorithm. This method calls upon several of the
	 * supporting algorithms in order to hash the plaintext and create the
	 * ciphertext tuple.
	 * 
	 * @param ID
	 *            - Identity String id
	 ...
	 * @return A ciphertext tuple (u, C_0, C_1, y) in F_q x E(F_p) x E(F_p) x
	 *         {0, ... , 255}^|m|
	 * @throws NoSuchAlgorithmException
	 * 
	 * 
	...
	public static ArrayList encryption(String PT, String ID, PublicParameter pp)
		...
		// System.out.println("C_O is on curve : " +
		// pp.sstateBB.getCurve().isOnCurve(C_0));
		...
		// System.out.println("C_1 is on curve : " +
		// pp.sstateBB.getCurve().isOnCurve(C_1));
		...
		byte[] pt = PT.getBytes();
		byte[] ybyte = SupportingAlgorithms.xorTwoByteArrays(pt,
				SupportingAlgorithms.HashBytes(pt.length, h_, pp.hashfcnBB));
		...
		BigInt rho = SupportingAlgorithms.HashToRange(h__, pp.qBB,
				pp.getHashfcnBB());
	...
	/**
	 * The decryption algorithm. This method calls upon several of the
	 * supporting algorithms in order to decrypt the ciphertext and return the
	 * plaintext message.
	 * 
	 * @param quad
	 *            - A ciphertext quadruple (u, C_0, C_1, y) in Z_q x E(F_p) x
	 *            E(F_p) x {0, ... , 255}*
	 * @param pp
	 *            - A set of public parameters
	 * @param privatekey
	 *            - A private key given as a pair of points (D_0, D_1) of order
	 *            q in E(F_p)
	 * @return A decrypted plaintext m, or an invalid ciphertext flag
	 * @throws NoSuchAlgorithmException
	 * 
	 * 
	 */
	public static String decryption(ArrayList quad, PublicParameter pp,
			ArrayList privatekey) throws NoSuchAlgorithmException {
		MessageDigest messageDigest = MessageDigest.getInstance(pp.hashfcnBB);
		...
		Complex d = (Complex) pp.sstateBB.compute(C_1, pp.getSstateBB()
				.getCurve2().negate(D_1));
		...
		byte[] m = SupportingAlgorithms.xorTwoByteArrays(ybyte,
				SupportingAlgorithms.HashBytes(ybyte.length, h_, pp.hashfcnBB));
		...
		BigInt rho = SupportingAlgorithms.HashToRange(h__, pp.getqBB(),
				pp.hashfcnBB);
	...
	/**
	 * Used by the encryption and decryption methods in order to create a new
	 * array which contains the seperate input arrays.
	 * 
	 * @param y_1
	 * @param x_1
	 * @param y_0
	 ...
	 * @param ybyte
	 * @param psi
	 * @return An array which is contains the six input arrays in a specific
	 *         manner.
	 * 
	 */
